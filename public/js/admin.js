...
async function loadChangelogEntriesForAdmin() {
  try {
    const response = await fetch('/changelog?page=cosmetic', { credentials: 'include' });
    const cosmeticLogs = await response.json();

    const response2 = await fetch('/changelog?page=noncosmetic', { credentials: 'include' });
    const nonCosmeticLogs = await response2.json();

    const allLogs = [...cosmeticLogs, ...nonCosmeticLogs].sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
    const container = document.getElementById('changelog-table-container');
    container.innerHTML = '';
    ...
    allLogs.forEach((entry) => {
      ...
      row.innerHTML = `
        <div><strong>${entry.username}</strong> <span class="role-tag ${entry.role}">${entry.role}</span></div>
        <div style="margin: 4px 0; font-size: 0.85rem;"><em>${new Date(entry.timestamp).toLocaleString()}</em> — <strong>[${entry.page}]</strong></div>
        <div style="margin: 6px 0;">${entry.message}</div>
        <div>
          <button class="edit-btn" data-id="${entry.id}" data-message="${entry.message}">Edit</button>
          ${userRole === 'SysAdmin' ? `<button class="delete-btn" data-id="${entry.id}">Delete</button>` : ''}
        </div>
        <hr />
      `;
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
      btn.onclick = () => {
        ...
        const oldMessage = btn.dataset.message;
        showGlobalModal({
          type: "warning",
          title: "Edit Changelog",
          message: `
            <textarea id="edit-changelog-textarea" style="width: 100%; height: 100px;">${oldMessage}</textarea>
          `,
          buttons: [
            {
              label: "Cancel",
              onClick: "fadeOutAndRemove('global-modal')"
            },
            {
              label: "Save",
              onClick: `
                const newMsg = document.getElementById('edit-changelog-textarea').value.trim();
                if (!newMsg) {
                  showGlobalModal({
                    type: "error",
                    title: "Invalid Input",
                    message: "Message cannot be empty.",
                    buttons: [{ label: "Close", onClick: "fadeOutAndRemove('global-modal')" }],
                    id: "global-modal"
                  });
                } else {
                  fetch('/admin/changelog/${id}', {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    credentials: 'include',
                    body: JSON.stringify({ message: newMsg })
                  }).then(res => res.json()).then(data => {
                    fadeOutAndRemove('global-modal');
                    loadChangelogEntriesForAdmin();
                  });
                }
              `
            }
        ...
        fetch(`/admin/changelog/${id}`, {
          ...
          credentials: 'include',
          body: JSON.stringify({ message: newMsg })
        }).then(res => res.json()).then(loadChangelogEntriesForAdmin);
      };
    });
    ...
    document.querySelectorAll('.delete-btn').forEach(btn => {
      btn.onclick = () => {
        ...
        showGlobalModal({
          ...
          buttons: [
            { label: "Cancel", onClick: "fadeOutAndRemove('global-modal')" },
            {
              label: "Delete",
              onClick: `
                fetch('/admin/changelog/${id}', {
...
document.getElementById('changelog-form')?.addEventListener('submit', async (e) => {
  ...
  const page = document.querySelector('input[name="crateType"]:checked').value;

  if (!message) {
    showGlobalModal({
      type: "error",
      title: "Missing Message",
      message: "Please enter a changelog message before submitting.",
      buttons: [{ label: "Close", onClick: "fadeOutAndRemove('global-modal')" }],
      id: "global-modal"
    });
    return;
  ...
  if (data.success) {
    ...
    showGlobalModal({
      ...
      message: "Your changelog entry was successfully saved.",
      buttons: [{ label: "Close", onClick: "fadeOutAndRemove('global-modal')" }],
      id: "global-modal"
    });
  } else {
    showGlobalModal({
      type: "error",
      title: "Submit Failed",
      message: data.error || "An unknown error occurred while submitting the changelog.",
      buttons: [{ label: "Close", onClick: "fadeOutAndRemove('global-modal')" }],
      id: "global-modal"
    });
  }
